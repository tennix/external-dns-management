...
func (this *Execution) add(dnsname string, rset *dns.RecordSet, modonly bool, found *RecordSet, notfound *RecordSet) {
	...
	for _, r := range rset.Records {
		...
		if old != nil {
			...
			if notfound != nil {
				nr := this.NewRecord(dnsname, rtype, r.Value, int(rset.TTL))
...
func (this *Execution) submitChanges() error {
	...
	for _, r := range this.additions {
		this.Infof("desired change: Addition %s %s: %s (%d)", r.DNSName(), r.Type(), r.Value(), r.TTL())
		this.submit(this.createRecord, r)
	}
	for _, r := range this.updates {
		this.Infof("desired change: Update %s %s: %s (%d)", r.DNSName(), r.Type(), r.Value(), r.TTL())
		this.submit(this.updateRecord, r)
	}
	for _, r := range this.deletions {
		this.Infof("desired change: Deletion %s %s: %s", r.DNSName(), r.Type(), r.Value())
		this.submit(this.deleteRecord, r)
	}
...
func (this *Execution) NewRecord(dnsname, rtype, value string, ttl int) Record {
	switch rtype {
	case dns.RS_A:
		return &RecordA{
			Name:     dnsname,
			Ipv4Addr: value,
			Zone:     this.zone.Key(),
			View:     *this.handler.infobloxConfig.View,
		}
	case dns.RS_CNAME:
		return &RecordCNAME{
			Name:      dnsname,
